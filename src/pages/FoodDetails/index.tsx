...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      ...
      const foodReceive: Food = response.data;

      setFood({
        ...foodReceive,
        formattedPrice: formatValue(foodReceive.price),
      });

      setExtras(
        foodReceive.extras.map((extra: Omit<Extra, 'quantity'>) => {
  ...
  const cartTotal = useMemo(() => {
    ...
    const extraTotal = extras.reduce(
      (acum, extra) => acum + extra.quantity * extra.value,
      0,
    );
  ...
  async function handleFinishOrder(): Promise<void> {
    ...
    const extrasUsed = extras.filter(extra => extra.quantity > 0);
